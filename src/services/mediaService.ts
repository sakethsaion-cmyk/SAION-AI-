...
export async function launchAndroidApp(appQuery: string): Promise<AppLaunchResult> {
  ...
  // Try to launch via intent URL — correct Android intent format
  return new Promise((resolve) => {
    ...
    // Build correct intent URL
    // Format: intent://[host]#Intent;scheme=[scheme];package=[pkg];S.browser_fallback_url=[fallback];end
    let launchUrl: string
    if (app.scheme.startsWith('intent:')) {
      launchUrl = app.scheme
    } else {
      const scheme = app.scheme.split('://')[0] || app.scheme.replace('://', '')
      const fallback = app.playStore
        ? encodeURIComponent(app.playStore)
        : encodeURIComponent(`https://play.google.com/store/apps/details?id=${app.package}`)
      launchUrl = `intent://open#Intent;scheme=${scheme};package=${app.package};S.browser_fallback_url=${fallback};end`
    }
    ...
    // After 2.5s check if app opened
    setTimeout(() => {
      document.removeEventListener('visibilitychange', onVisibilityChange)
      ...
      if (appOpened) {
        ...
        // App not installed — show Play Store link
        resolve({
          attempted: true,
          isAndroid: true,
          appName: app.label,
          method: 'store',
          message: `📱 **${app.label}** doesn't appear to be installed on your device.${app.playStore ? `\n\n[📥 Download ${app.label} from Play Store](${app.playStore})` : ''}`,
        })
      }
    }, 2500)
  })
