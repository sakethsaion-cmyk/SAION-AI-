...
import { Globe, X, Loader2, Shield } from "lucide-react";
...
const FEATURES = [
  {
    icon: "🤖",
    title: "AI Chat",
    desc: "Talk to SAION AI in any language — English, Hindi, Telugu and more. Smart, fast, context-aware.",
...
export default function DownloadPage() {
  ...
  const { signInWithGoogle, currentUser, isBanned, banReason } = useAuth();
  const [showWebSignIn, setShowWebSignIn] = useState(false);
  const [signingIn, setSigningIn] = useState(false);
  const [signInError, setSignInError] = useState<string | null>(null);

  // ── Auto-redirect after sign-in (handles both popup + redirect flow) ───────
  useEffect(() => {
    if (currentUser) {
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      <nav className="relative z-10 max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 border border-violet-600/40 hover:border-violet-500 bg-violet-900/20 hover:bg-violet-900/30 text-violet-300 text-sm font-medium rounded-xl transition-all"
          >
            <Globe size={15} /> Full Web Access
          </button>
        </div>
      </nav>
      ...
      >
        ...
        <div className="mb-8 p-6 rounded-3xl border border-violet-600/30 bg-gradient-to-br from-violet-900/20 to-[#0a0a0a] text-center fade-in-up">
          <div className="flex items-center justify-center gap-2 mb-3">
            <Globe size={20} className="text-violet-400" />
            <h3 className="text-lg font-bold text-white">Try SAION AI in Your Browser</h3>
            <span className="text-xs bg-violet-700/40 border border-violet-600/30 text-violet-300 px-2 py-0.5 rounded-full">No download needed</span>
          </div>
          <p className="text-gray-500 text-sm mb-5 max-w-md mx-auto">
            Access the full SAION AI experience directly in your browser. Sign in with Google and start chatting instantly.
          </p>
          <button
            onClick={handleWebAccess}
            className="inline-flex items-center gap-2.5 px-8 py-3.5 rounded-2xl bg-violet-600 hover:bg-violet-500 text-white font-bold text-sm transition-all hover:scale-[1.02] shadow-[0_0_30px_rgba(124,58,237,0.4)] hover:shadow-[0_0_40px_rgba(124,58,237,0.6)]"
          >
            <Globe size={16} />
            {currentUser ? 'Open SAION AI' : 'Sign In & Start Chatting'}
          </button>
          <p className="text-gray-700 text-xs mt-3">Sign in with Google · Works on any device · Full features</p>
        </div>
    ...
    <div className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      {showWebSignIn && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
          <div className="relative w-full max-w-sm bg-[#0c0c0c] border border-[#1e1e1e] rounded-3xl p-8 shadow-2xl shadow-violet-900/20 fade-in-up">
            <button onClick={() => { setShowWebSignIn(false); setSignInError(null); }}
            ...
            <h2 className="font-display text-2xl font-bold text-white text-center mb-1">Welcome to SAION AI</h2>
            <p className="text-gray-500 text-sm text-center mb-8">Sign in to access the full AI experience in your browser</p>
            <button
              onClick={handleGoogleSignIn}
              disabled={signingIn}
              className="w-full flex items-center justify-center gap-3 py-3.5 px-5 rounded-xl bg-white text-gray-900 font-semibold text-sm hover:bg-gray-100 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {signingIn ? (
            ...
            {signInError && <p className="mt-4 text-red-400 text-xs text-center">{signInError}</p>}
            <p className="mt-6 text-gray-700 text-xs text-center">By signing in you agree to our Terms of Service.<br />Your data is encrypted and never shared.</p>
          </div>
        </div>
      )}
      ...
      {isBanned && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black px-4">
          <div className="w-full max-w-sm text-center">
            ...
            <p className="text-gray-500 text-sm mb-4">{banReason || 'Your account has been suspended from SAION AI.'}</p>
            <p className="text-gray-700 text-xs">If you believe this is a mistake, contact support.</p>
          </div>
        </div>
      )}
    </div>
