...
import { incrementMessageCount, updateConversation } from "../../services/dbService";
import { YoutubeModule } from "../../modules/youtube";
import type { YoutubeVideoResult } from "../../modules/youtube";
import { getMemories, formatMemoriesForPrompt } from "../../modules/memory/memoryService";
import { runCode, isRunCodeRequest } from "../../modules/codeRunner/codeRunner";
...
export default function ChatWindow() {
  ...
  const [showYoutubeModule, setShowYoutubeModule] = useState(false);
  const [youtubeInitialQuery, setYoutubeInitialQuery] = useState("");
  ...
  const loadMemoryContext = useCallback(async () => {
    ...
    try {
      const mems = await getMemories(currentUser.uid);
      setMemoryContext(formatMemoriesForPrompt(mems));
    } catch { /* non-critical */ }
  }, [currentUser?.uid]);

  useEffect(() => { loadMemoryContext(); }, [loadMemoryContext]);
  ...
  const handleEditMessage = async (messageId: string, newContent: string) => {
    ...
    if (idx === -1) return;
    const updatedMsgs = msgs.slice(0, idx).concat({ ...msgs[idx], content: newContent });
    await updateConversation(activeConversation.id, updatedMsgs);
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0 h-full bg-black">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-[#141414] bg-[#060606] shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-gray-500 hover:text-violet-400 hover:bg-violet-900/10 rounded-lg text-xs transition-all"
      ...
      {imageStatus && (
        <div className="tool-card-animate border-b border-[#111] bg-[#060606] px-4 py-2.5 flex items-center gap-3">
          <div className="w-5 h-5 spin-ring shrink-0" />
          <div className="flex-1">
            <span className="text-xs text-violet-400 font-medium">{imageStatus}</span>
          </div>
          <div className="flex gap-1 items-center">
            {['#7c3aed','#8b5cf6','#a78bfa','#7c3aed'].map((color, i) => (
              <div key={i} className="w-1 h-3 rounded-full animate-pulse" style={{background: color, animationDelay: `${i*100}ms`}} />
            ))}
          </div>
        </div>
      )}
      ...
      {searchStatus && (
        <div className="tool-card-animate border-b border-[#111] bg-[#060606] px-4 py-2.5 flex items-center gap-3">
          <div className="relative search-scan w-5 h-5 flex items-center justify-center overflow-hidden">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#7c3aed" strokeWidth="2">
              <circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/>
            </svg>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs text-violet-400 font-medium">{searchStatus}</span>
            <div className="flex gap-1">
              ...
              <div className="w-1 h-1 rounded-full bg-violet-300 animate-bounce" style={{animationDelay:'300ms'}} />
            </div>
          </div>
          <div className="ml-auto text-[10px] text-gray-700">🦆 DuckDuckGo</div>
        </div>
      )}
        ...
        <div className="border-t border-[#1a1a1a] bg-[#080808]">
          <div className="flex items-center justify-between px-4 py-2 border-b border-[#141414]">
            <span className="text-xs text-gray-400 flex items-center gap-2">
              <svg viewBox="0 0 24 24" width="14" height="14" fill="#FF0000"><path d="M23.5 6.19a3.02 3.02 0 0 0-2.12-2.14C19.54 3.5 12 3.5 12 3.5s-7.54 0-9.38.55A3.02 3.02 0 0 0 .5 6.19C0 8.04 0 12 0 12s0 3.96.5 5.81a3.02 3.02 0 0 0 2.12 2.14C4.46 20.5 12 20.5 12 20.5s7.54 0 9.38-.55a3.02 3.02 0 0 0 2.12-2.14C24 15.96 24 12 24 12s0-3.96-.5-5.81z"/><polygon fill="white" points="9.75,15.02 15.5,12 9.75,8.98"/></svg>
              YouTube
            </span>
            <button onClick={() => setShowYoutubeModule(false)} className="text-gray-600 hover:text-white p-1">
              <X size={14} />
            </button>
          </div>
          <div className="p-4 max-h-[55vh] overflow-y-auto">
            <YoutubeModule
              ...
              onVideoSelect={(video: YoutubeVideoResult) => {
                addMessage({ role: "assistant", content: `▶️ Now playing: **${video.title}** by ${video.channelTitle}`, type: "youtube" });
              }}
            />
          </div>
        </div>
      ...
      {isStreaming && (
        <div className="flex items-center gap-2 px-4 py-1.5 bg-black border-t border-violet-900/30">
          <div className="w-1.5 h-1.5 rounded-full bg-violet-500 status-dot-live" />
          <span className="text-[11px] text-violet-400">SAION AI is responding…</span>
          <div className="flex gap-0.5 ml-1">
            {[0,1,2,3,4].map(i => (
              <div key={i} className="w-0.5 rounded-full bg-violet-500 animate-pulse"
      ...
      {/* Memory Panel */}
      {showMemoryPanel && (
        <MemoryPanel onClose={() => { setShowMemoryPanel(false); loadMemoryContext(); }} />
      )}
